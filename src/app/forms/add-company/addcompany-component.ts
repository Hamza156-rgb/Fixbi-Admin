...
export class AddCompanyComponent implements OnInit {
	...
	ngOnInit() {
		setTimeout(() => {
			this.pageTitleService.setTitle("Add New Company");
		}, 0);

		this.form = this.fb.group({
			company_name: [null, Validators.compose([Validators.required, Validators.minLength(2), Validators.maxLength(100)])],
		...
		if (this.form.status == 'VALID') {
			this.config.postHttp('auth/registerProfessional', this.form.value).then((data: any) => {
				if (data.success == 0) {
					...
					this.router.navigate(['tables/companies']);
				}
			})
		}
	}
