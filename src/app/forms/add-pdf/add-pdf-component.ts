...
export class AddPDFComponent implements OnInit {
	...
	ngOnInit() {
		setTimeout(() => {
			this.pageTitleService.setTitle("Add New PDF File");
		}, 0);
	...
	onSubmit() {
		...
		if (this.form.status == 'VALID') {
			...
			this.config.postHttp('pdf/createPdfFile', myFormData).then((data: any) => {
				if (data.success == 0) {
					this.toastr.error(data.message);
					return;
				} 
				else {
					this.toastr.success('PDF Created Successfully!');
					this.router.navigate(['tables/pdf']);
				}
			})
