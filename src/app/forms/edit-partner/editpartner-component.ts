...
export class EditPartnerComponent implements OnInit {
	...
	ngOnInit() {
		setTimeout(() => {
			this.pageTitleService.setTitle("Edit Partner");
		}, 0);
		...
		this.config.getHttp('partner/getPartnersById', { params: param }).then((data: any) => {
	...
	onSubmit() {
		...
		if (this.form.status == 'VALID') {
			...
			this.config.postHttp('partner/updatePartners', myFormData).then((data: any) => {
				if (data.success == 0) {
